...
class SettingsPage {
    ...
    initVpnSettings() {
        const settings = this.app.player.settings;

        const hostInput = document.getElementById('setting-warp-host');
        const portInput = document.getElementById('setting-warp-port');
        const rulesTextarea = document.getElementById('setting-warp-rules');
        const saveBtn = document.getElementById('save-vpn-settings');
        const testBtn = document.getElementById('test-vpn-settings');
        const testResult = document.getElementById('vpn-test-result');

        if (!hostInput || !saveBtn) return;

        // Load current values
        hostInput.value = settings.warpHost || '127.0.0.1';
        portInput.value = settings.warpPort || 1080;
        rulesTextarea.value = (settings.warpProxyRules || []).join('\n');

        const autoIpBtn = document.getElementById('auto-detect-ip');
        if (autoIpBtn) {
            autoIpBtn.onclick = async () => {
                autoIpBtn.disabled = true;
                this.app.showToast('Detecting server IP...', 'info');
                try {
                    ...
                    if (data.ip) {
                        ...
                        this.app.showToast('IP detected: ' + data.ip, 'success');
                    }
                } catch (err) {
                    console.error('Failed to detect IP:', err);
                    this.app.showToast('Failed to detect IP', 'error');
                } finally {
                    autoIpBtn.disabled = false;
                }
            };

            // Auto-fill if it's still default 127.0.0.1
            if (hostInput.value === '127.0.0.1' || !hostInput.value) {
                fetch('/api/settings/ip', {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` }
                }).then(r => r.json()).then(data => {
                    if (data.ip && data.ip !== '127.0.0.1') {
                        hostInput.value = data.ip;
                        console.log('[Settings] Auto-detected local IP:', data.ip);
                    }
                }).catch(() => { });
            }
        }
        ...
        testBtn.onclick = async () => {
            ...
            const port = parseInt(portInput.value.trim()) || 1080;

            if (!host) {
                this.app.showToast('Please enter a host', 'error');
                return;
            }
            ...
            try {
                const response = await fetch('/api/settings/test-vpn', {
                    method: 'POST',
                    headers: {
                ...
                try {
                    ...
                    console.error('Failed to parse test response:', text);
                    throw new Error('Server returned non-JSON response. Please restart the server.');
                }

                if (response.ok && data.success) {
                    testResult.textContent = '✅ Success! Info: ' + data.info;
                    testResult.style.color = 'var(--color-success)';
                    this.app.showToast('VPN Connection successful', 'success');
                } else {
                ...
                this.app.showToast('VPN Connection failed', 'error');
            } finally {
        ...
        saveBtn.onclick = async () => {
            const updates = {
                warpHost: hostInput.value.trim(),
                warpPort: parseInt(portInput.value.trim()) || 1080,
                warpProxyRules: rulesTextarea.value.split('\n').map(r => r.trim()).filter(r => r !== '')
            };
            ...
            try {
                ...
                Object.assign(this.app.player.settings, updates);

                this.app.showToast('VPN settings saved successfully', 'success');
            } catch (err) {
                console.error('Failed to save VPN settings:', err);
                this.app.showToast('Error saving VPN settings: ' + err.message, 'error');
            } finally {
                saveBtn.disabled = false;
                saveBtn.textContent = 'Save VPN Settings';
            }
        };
    }
