...
router.post('/drm', express.raw({ type: '*/*', limit: '10mb' }), async (req, res) => {
    try {
        const { url } = req.query;
        if (!url) {
            return res.status(400).json({ error: 'DRM URL required' });
        }
...
router.get('/stream', async (req, res) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // --- EMBEDDED HEADERS EXTRACTION ---
            // If the URL has a ?headers= Base64 query parameter, it often contains critical 
            // auth tokens (dazn-token, etc.) that the proxy MUST forward.
            try {
                const urlObj = new URL(url);
                const embeddedHeadersB64 = urlObj.searchParams.get('headers');
                if (embeddedHeadersB64) {
                    try {
                        const decoded = Buffer.from(embeddedHeadersB64, 'base64').toString('utf8');
                        const jsonHeaders = JSON.parse(decoded);
                        console.log(`[Proxy] Merging ${Object.keys(jsonHeaders).length} embedded headers from URL query`);

                        const skipHeaders = ['host', 'connection', 'content-length', 'accept-encoding', 'cookie'];
                        ...
                        console.warn('[Proxy] Failed to decode/parse embedded headers:', e.message);
                    }
                }
            } catch (e) { /* Invalid URL or missing protocol - ignore */ }
