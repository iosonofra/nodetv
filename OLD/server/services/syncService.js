...
class SyncService {
    ...
    async syncXtream(source) {
        ...
        const db = getDb();
        const proxyAgent = proxyUrl ? new (require('socks-proxy-agent').SocksProxyAgent)(proxyUrl) : null;
        ...
        try {
            ...
            await this.syncEpgFromUrl(source.id, xmltvUrl, proxyAgent);
        } catch (e) {
     ...
     */
    async syncEpgFromUrl(sourceId, url, proxyAgent = null) {
        ...
        for await (const batch of epgParser.fetchAndParseStreaming(url, 1000, proxyAgent)) {
            batchCount++;
    ...
    async syncM3u(source) {
        ...
        console.log(`[Sync] Fetching M3U playlist for ${source.name}${proxyUrl ? ' via Warp' : ''}`);
        ...
        for await (const batch of m3uParser.fetchAndParseStreaming(source.url, 500, proxyAgent)) {
            batchCount++;
    ...
    async syncEpg(source) {
        ...
        console.log(`[Sync] Fetching standalone EPG for ${source.name}${proxyUrl ? ' via Warp' : ''}`);
        await this.syncEpgFromUrl(source.id, source.url, proxyAgent);
    }
}
