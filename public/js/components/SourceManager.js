...
class SourceManager {
      ...
      <div class="source-item ${source.enabled ? '' : 'disabled'}" data-id="${source.id}">
        ...
        <div class="source-info" style="flex: 1; min-width: 0;">
          <div class="source-name" style="display: flex; align-items: center; gap: 8px;">
            <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${source.name}</span>
            ${source.useWarp ? '<span class="version-badge" style="background: var(--color-bg-tertiary); color: var(--color-accent); border: 1px solid var(--color-accent-dim); font-size: 0.6rem; padding: 2px 6px; border-radius: 4px; flex-shrink: 0;">WARP</span>' : ''}
          </div>
          <div class="source-url" style="font-size: 0.8rem; color: var(--color-text-muted); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${source.url}</div>
        </div>
    ...
    `).join('');
        ...
        container.querySelectorAll('.source-item').forEach(item => {
            ...
            console.log(`[SourceManager] Attaching listeners for source ${id} (${type})`);

            item.querySelector('[data-action="refresh"]').onclick = (e) => { e.stopPropagation(); this.refreshSource(id, type); };
            item.querySelector('[data-action="test"]').onclick = (e) => { e.stopPropagation(); this.testSource(id); };
            item.querySelector('[data-action="toggle"]').onclick = (e) => { e.stopPropagation(); this.toggleSource(id); };
            item.querySelector('[data-action="edit"]').onclick = (e) => { e.stopPropagation(); this.showEditModal(id, type); };
            item.querySelector('[data-action="delete"]').onclick = (e) => { e.stopPropagation(); this.deleteSource(id); };
        });
    }
    ...
    async saveNewSource(type) {
        ...
        try {
            ...
            if (type === 'm3u') {
                try {
                    const estimate = await API.sources.estimateByUrl(url, type, useWarp);
