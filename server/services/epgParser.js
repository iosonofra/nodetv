 ...
 */
async function fetchAndParse(url, proxyAgent = null) {
    const fetchOptions = {
        timeout: 60000 // 60s timeout for large EPG files
    };
    if (proxyAgent) {
 ...
 */
async function* fetchAndParseStreaming(url, batchSize = 1000, proxyAgent = null) {
    const fetchOptions = {
        timeout: 60000 // 60s timeout for large EPG files
    };
    if (proxyAgent) {
