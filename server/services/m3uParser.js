 ...
 */
async function getInputStream(urlOrPath, proxyAgent = null) {
    if (urlOrPath.startsWith('http://') || urlOrPath.startsWith('https://')) {
        ...
        const response = await fetch(urlOrPath, fetchOptions);
        if (!response.ok) {
 ...
 * @returns {Promise<{ channels: Array, groups: Array }>}
 */
async function fetchAndParse(urlOrPath, proxyAgent = null) {
    const stream = await getInputStream(urlOrPath, proxyAgent);
    return parse(stream);
}
 ...
 * @yields {{ channels: Array, groups: Set, isLast: boolean }}
 */
async function* fetchAndParseStreaming(urlOrPath, batchSize = 500, proxyAgent = null) {
    const stream = await getInputStream(urlOrPath, proxyAgent);
 ...
 */
async function countEntries(urlOrPath, proxyAgent = null) {
    const stream = await getInputStream(urlOrPath, proxyAgent);
