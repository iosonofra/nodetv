...
class SportzxService {
    constructor() {
        ...
        this.historyFile = path.join(this.dataDir, 'sportzx_history.json');
        this.playlistFile = path.join(this.dataDir, 'sportzx.m3u');
        this.scriptPath = path.join(__dirname, '../../Sportzx.py');
    ...
    async getStatus() {
        ...
        const intervalHours = parseInt(currentSettings.sportzxInterval) || 1;
        const autoRunEnabled = currentSettings.sportzxAutoRun === true;
        const timeoutSeconds = parseInt(currentSettings.sportzxTimeout) || 15;
        const excludedCategories = currentSettings.sportzxExcludedCategories || ['adult', 'test', 'xxx'];
    ...
    async run(runType = null) {
        if (this.isRunning) {
            throw new Error('Sportzx scraper is already running');
        }

        if (!fs.existsSync(this.scriptPath)) {
            throw new Error(`Sportzx script not found at ${this.scriptPath}`);
        }
        ...
        this.addLog(`[*] Starting Sportzx scraper execution (${runType})...`);
        this.addLog(`[*] Excluded categories: ${excludedCategories.join(', ') || '(none)'}`);
        ...
        this.currentProcess = spawn(python.cmd, args, {
            env: {
                ...
                SPORTZX_OUTPUT: this.playlistFile,
                SPORTZX_TIMEOUT: String(timeoutSeconds),
                SPORTZX_EXCLUDED_CATEGORIES: JSON.stringify(excludedCategories)
            }
        });
        ...
        this.currentProcess.stderr.on('data', (data) => {
            ...
            lines.forEach(line => {
                if (line.trim()) {
                    ...
                    console.error(`[Sportzx Error] ${line.trim()}`);
                }
            });
        });
        ...
        return new Promise((resolve, reject) => {
            ...
            const executionTimeout = setTimeout(() => {
                if (this.currentProcess && this.isRunning) {
                    this.addLog('[CRITICAL] Sportzx scraper timed out (20m). Killing process.');
                    this.currentProcess.kill('SIGKILL');
                    this.isRunning = false;
                    this.currentProcess = null;
                    reject(new Error('Sportzx scraper execution timeout'));
                }
            }, TIMEOUT_MS);
            ...
            this.currentProcess.on('exit', async (code) => {
                ...
                if (code === 0) {
                    this.addLog('[v] Sportzx scraper completed successfully.');
                    ...
                    this.addLog(`[!] Sportzx scraper exited with code ${code}`);
                    this.saveHistoryEntry({
                        timestamp: new Date().toISOString(),
                        runType,
                        success: false,
    ...
    async ensureSourceRegistered() {
        ...
        const existingSource = allSources.find(s => s.name === 'Sportzx Events');

        if (existingSource) {
            const expectedUrl = 'data/scraper/sportzx.m3u';
            const updates = {};
    ...
    async startAutoRun() {
        const currentSettings = await settings.get();
        const intervalHours = parseInt(currentSettings.sportzxInterval) || 1;
        const enabled = currentSettings.sportzxAutoRun === true;

        if (!enabled) {
            console.log('[Sportzx] Auto-run is disabled in settings');
            this.stopAutoRun();
            return;
        }
        ...
        console.log(`[Sportzx] Starting auto-run every ${intervalHours} hours`);
        ...
        if (timeSinceLastRun >= intervalMs) {
            console.log('[Sportzx] Triggering immediate run on startup (interval passed)');
            this.run('auto').catch(err => {
                console.error('[Sportzx] Startup run failed:', err);
            });
            ...
            console.log(`[Sportzx] Next run scheduled in ${Math.round(waitTime / 60000)} minutes`);
    ...
    _triggerScheduledRun() {
        if (!this.isRunning) {
            console.log('[Sportzx] Triggering scheduled run...');
            this.run('auto').catch(err => {
                console.error('[Sportzx] Scheduled run failed:', err);
            });
        } else {
            console.log('[Sportzx] Scheduled run skipped (already running)');
        }
    }
    ...
    stopAutoRun() {
        if (this._autoRunTimer) {
            clearInterval(this._autoRunTimer);
            this._autoRunTimer = null;
            console.log('[Sportzx] Auto-run disabled');
        }
    }
}

module.exports = new SportzxService();
